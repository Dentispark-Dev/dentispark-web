...
DO $$ 
BEGIN 
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='platform_scholarships' AND column_name='provider') THEN
        ALTER TABLE platform_scholarships ADD COLUMN provider VARCHAR(255);
    END IF;
END $$;
...
INSERT INTO platform_scholarships (
    ...
    deadline, application_link, is_sponsored, target_degree_level, target_location, provider,
...
(
    ...
    true, 'BDS', 'United Kingdom', 'NHS (England)',
    '{"notes": "Means-tested; automatic eligibility for students in later years."}', now(), now()
),
(
    ...
    true, 'BDS', 'United Kingdom', 'NHS',
    '{"notes": "Apply yearly after enrolment. For healthcare students incl. dentistry."}', now(), now()
),
(
    ...
    false, 'BDS', 'United Kingdom', 'QMUL',
    '{"notes": "Auto-assessed based on Student Finance application."}', now(), now()
),
(
    ...
    false, 'BDS', 'United Kingdom', 'All UK Universities',
    '{"notes": "Early deadline requirement for all aspiring dental students."}', now(), now()
),
(
    ...
    false, 'BDS', 'United Kingdom', 'Newcastle University',
    '{"notes": "Income-based; no separate application required."}', now(), now()
),
(
    ...
    false, 'BDS', 'United Kingdom', 'Newcastle University',
    '{"notes": "Course-specific funds for dental students."}', now(), now()
),
(
    ...
    false, 'BDS', 'United Kingdom', 'University of Dundee',
    '{"notes": "Specifically for non-Scottish UK residents."}', now(), now()
),
(
    ...
    true, 'BDS', 'United Kingdom', 'University of Dundee',
    '{"notes": "Grade-based award for high achievers."}', now(), now()
),
(
    ...
    false, 'BDS', 'United Kingdom', 'University of East Anglia',
    '{"notes": "Competitive application process."}', now(), now()
),
(
    ...
    false, 'BDS', 'United Kingdom', 'External',
    '{"notes": "Exclusively for military families."}', now(), now()
),
(
    ...
    false, 'BDS', 'Global', 'External',
    '{"notes": "Dedicated to international students."}', now(), now()
),
(
    ...
    false, 'BDS', 'United Kingdom', 'Kingston University',
    '{"notes": "Offer of study required before application."}', now(), now()
)
