...
interface MentorVerifyEmailPageProps {
  searchParams: Promise<{
    email?: string;
  }>;
}

export default function MentorVerifyEmailPage({
  searchParams,
}: MentorVerifyEmailPageProps) {
  const [timeLeft, setTimeLeft] = useState(59);
  const verifyEmailMutation = useVerifyEmail();
  const resendCodeMutation = useResendCode();
  const resolvedSearchParams = use(searchParams);
  const email = resolvedSearchParams.email || "mentor@dentispark.co.uk";
  ...
  const onSubmit = async (data: VerifyEmailFormData) => {
    try {
      router.push("/mentor/new-password");
