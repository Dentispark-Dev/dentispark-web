    ...
    if (isLoading) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[600px] gap-6 bg-white font-jakarta">
                ...
                <p className="text-slate-400 font-black uppercase tracking-[0.3em] text-[10px]">Authorizing Network Access...</p>
            </div>
        );
    }
        ...
        return (
            <div className="flex flex-col items-center justify-center min-h-[500px] space-y-8 text-center max-w-xl mx-auto bg-white p-16 rounded-[3rem] border border-slate-100 shadow-2xl font-jakarta">
                ...
                <div className="space-y-3">
                    <h3 className="text-2xl font-black text-slate-900 tracking-tight">Identity Synchronization Failure</h3>
                ...
                <Button 
                    onClick={() => router.back()}
                    variant="outline" 
                    className="rounded-[1.5rem] px-12 h-14 border-slate-200 font-black text-xs uppercase tracking-widest transition-all hover:bg-slate-50 shadow-sm"
                >
                    Return to Hub
                </Button>
            </div>
            ...
            <div className="bg-white p-10 md:p-16 border-b border-slate-50 flex flex-col md:flex-row gap-12 justify-between items-center relative overflow-hidden">
                    ...
                    <div className="space-y-5 text-center md:text-left flex-1 min-w-0">
                        <div>
                            <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 mb-3">
                                <Badge variant="outline" className="bg-teal-50 text-teal-600 border-teal-200 px-4 py-1.5 font-black text-[10px] tracking-[0.25em] rounded-full uppercase">
                                    Architect Entity
                                </Badge>
                                <span className="text-[11px] font-black text-slate-400 uppercase tracking-widest opacity-60">Namespace: @{admin.username}</span>
                            </div>
                ...
                <div className="flex flex-col sm:flex-row gap-4 relative z-10 shrink-0">
                    <Button 
                        variant="outline" 
                        onClick={() => router.back()}
                        className="h-14 px-8 rounded-2xl border-slate-100 bg-white text-slate-400 font-black text-xs gap-3 hover:bg-slate-50 hover:text-slate-900 transition-all active:scale-95 shadow-sm uppercase tracking-widest"
                    >
                        <ArrowLeft className="h-5 w-5" />
                        Exit Dossier
                    </Button>
                    <Button className="h-14 px-10 rounded-2xl bg-teal-600 text-white font-black text-xs hover:bg-teal-700 transition-all active:scale-95 shadow-xl shadow-teal-100 uppercase tracking-widest gap-3">
                ...
                <div className="flex flex-wrap gap-2 p-2 bg-slate-50 rounded-[2rem] w-fit border border-slate-100 mb-12 shadow-inner">
                    {tabs.map((tab) => (
                        <button
                            ...
                            className={cn(
                                "flex items-center gap-3 px-8 py-4 rounded-[1.5rem] text-[10px] font-black transition-all duration-300 uppercase tracking-widest",
                                activeTab === tab.key
                            ...
                            <div className="lg:col-span-8 space-y-10">
                                ...
                                <div className="bg-white rounded-[2.5rem] border border-slate-100 p-10 shadow-sm relative overflow-hidden group">
                                    ...
                                    <div className="flex items-center gap-3 mb-8">
                                        ...
                                        <h3 className="text-2xl font-black text-slate-900 tracking-tight">Identity Context</h3>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100">
                                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 opacity-60">Assigned Domain</p>
                                            <p className="text-sm font-black text-slate-900">Institutional Governance</p>
                                        </div>
                                        <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100">
                                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 opacity-60">Activation Date</p>
                                            <p className="text-sm font-black text-slate-900">January 12, 2024</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="lg:col-span-4 space-y-10">
                                ...
                                <div className="bg-slate-900 rounded-[2.5rem] p-10 text-white space-y-8 shadow-2xl relative overflow-hidden">
                                     ...
                                     <div className="space-y-4 relative z-10">
                                         <Button 
                                            className="w-full h-14 bg-white hover:bg-slate-100 text-slate-900 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] shadow-xl transition-all active:scale-95"
                                         ...
                                         <Button 
                                            variant="outline" 
                                            className="w-full h-14 border-white/10 hover:border-white/40 bg-transparent text-white rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] transition-all"
                                ...
                                <div className="bg-white rounded-[2.5rem] border border-slate-100 p-10 space-y-8">
                                    <div className="flex items-center justify-between">
                                        <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest opacity-60">Security State</h3>
                                    ...
                                    <div className="space-y-6">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">MFA Status</p>
                                                <p className="text-sm font-black text-slate-900">Biometric Verified</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    {activeTab === "governance" && (
                        ...
                        >
                            ...
                            <div className="relative z-10 flex flex-col md:flex-row gap-10 justify-between items-start">
                                <div className="space-y-3">
                                    <h3 className="text-4xl font-black text-slate-900 tracking-tight leading-tight">Role Intelligence Grid</h3>
                                ...
                                {admin.rolesAndPermissions?.map((role, i) => (
                                    <div key={role.guid} className="p-8 bg-slate-50 border border-slate-100 rounded-[2.5rem] group hover:bg-white hover:shadow-2xl hover:shadow-teal-100/50 transition-all duration-500">
                                        <div className="flex items-center gap-4 mb-6">
                                            ...
                                            <div>
                                                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1 opacity-60">System Role</p>
                                                <h4 className="text-lg font-black text-slate-900 leading-tight group-hover:text-teal-600 transition-colors">{role.name}</h4>
                                            </div>
                                        </div>
                                        <div className="pt-6 border-t border-slate-100">
                                            <Button variant="link" className="p-0 h-auto text-[10px] font-black text-slate-400 hover:text-teal-600 uppercase tracking-widest flex items-center gap-2 group/btn">
    ...
    return (
        <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-md transition-shadow group">
            <div className="flex items-center gap-3 mb-2">
                ...
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest opacity-60">{label}</p>
            </div>
            <p className="text-lg font-black text-slate-900 ml-9">{value}</p>
        </div>
    );
}
