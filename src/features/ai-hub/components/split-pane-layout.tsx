...
export function LelandSplitPane({ leftContent, rightContent, step }: LelandSplitPaneProps) {
  ...
  const getGlowColor = (s: number) => {
    switch(s) {
      case 0: return "rgba(20, 184, 166, 0.15)"; // Teal
      case 1: return "rgba(59, 130, 246, 0.15)"; // Blue
      case 2: return "rgba(139, 92, 246, 0.15)"; // Purple
      default: return "rgba(20, 184, 166, 0.15)";
    }
  };

  return (
    <div className="fixed inset-0 flex flex-col lg:flex-row bg-[#020617] overflow-hidden z-[50]">
      ...
      <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
        <motion.div 
          ...
          className="absolute top-[-20%] left-[-10%] w-[60%] h-[60%] blur-[160px] rounded-full opacity-40"
        />
        <motion.div 
          animate={{ 
            backgroundColor: step === 1 ? "rgba(236, 72, 153, 0.1)" : "rgba(30, 64, 175, 0.1)",
            scale: [1, 1.2, 1],
            x: [0, -100, 0],
            y: [0, 100, 0]
          }}
          ...
          className="absolute bottom-[-10%] right-[-10%] w-[50%] h-[50%] blur-[140px] rounded-full opacity-30"
        />
      ...
      <motion.div 
        className="w-full lg:w-1/2 h-full relative z-10 flex flex-col justify-center px-8 lg:px-24 py-12 overflow-y-auto border-r border-white-[0.03] bg-black/10 backdrop-blur-[2px]"
