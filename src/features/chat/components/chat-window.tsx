...
export function ChatWindow() {
  ...
  return (
    <div className="flex-1 h-full flex flex-col bg-gray-50/50 relative overflow-hidden font-sora">
          ...
          return (
            ...
            >
              <div className={cn(
                ...
                {/* Simplified Bubble Tail */}
                {isFirstFromSender && (
                  <div className={cn(
                    "absolute top-0 w-2.5 h-3",
                    isMe 
                      ? "-right-2 bg-emerald-600 [clip-path:polygon(0_0,0_100%,100%_0)]" 
                      : "-left-2 bg-white border-l border-t border-gray-100 [clip-path:polygon(100%_0,100%_100%,0_0)]"
                  )} />
                )}
      ...
      {/* Input */}
      <div className="p-4 bg-white border-t border-gray-100 relative z-10 shadow-[0_-4px_20px_rgba(0,0,0,0.03)]">
        <div className="flex items-center gap-3 max-w-[1200px] mx-auto">
          <div className="flex items-center">
            <button className="p-2.5 hover:bg-gray-50 rounded-xl transition-colors text-gray-400"><Paperclip className="w-6 h-6" /></button>
          </div>
          
          <div className="flex-1 bg-gray-50/80 rounded-2xl px-5 flex items-center border border-gray-100 focus-within:bg-white focus-within:ring-4 focus-within:ring-emerald-500/5 transition-all">
            <input 
              type="text"
              value={inputText}
              ...
              className="w-full bg-transparent py-4 text-[14px] font-medium focus:outline-none placeholder:text-gray-400 text-gray-800"
            />
          </div>
          ...
          <button 
              onClick={handleSend}
              className={cn(
                  "w-12 h-12 rounded-2xl flex items-center justify-center transition-all active:scale-95 flex-shrink-0 shadow-lg",
                  inputText.trim() ? "bg-emerald-600 text-white shadow-emerald-200" : "bg-gray-100 text-gray-300"
              )}
